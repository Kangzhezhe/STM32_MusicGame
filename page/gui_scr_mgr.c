...
typedef struct scr_mgr_reg_scr_list_handle_t {
    uint32_t              ScrCnt;
    SCR_MGR_SCR_HANDLE_T *ScrHandles[1];
...
void ScrMgrInit(void)
{
    ...
    ScrMgrRegScrList   = lv_malloc(sizeof(SCR_MGR_REG_SCR_LIST_HANDLE_T) + cnt * sizeof(SCR_MGR_SCR_HANDLE_T *));
...
bool ScrMgrSwitchScr(uint32_t scr_id, bool anim)
{
    ...
    if (ScrMgrScrStackTop != NULL) {
        ...
        lv_free((void *)ScrMgrScrStackTop);
    ...
    stack_item            = lv_malloc(sizeof(SCR_MGR_SCR_STACK_HANDLE_T));
